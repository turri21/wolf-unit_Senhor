...
architecture hdl of hc55564 is 
  ...
  -- State registers carry explicit initial values (Arcade-SmashTV): the
  -- original had no reset ("reset ??" below), so in 4-state simulation the
  -- integrator/syllabic arithmetic stayed 'U'/'X' forever and the decoder
  -- never produced samples. FPGA registers power up to 0 anyway, so this
  -- matches hardware behaviour; synthesis result is unchanged.
  signal runofn_new : std_logic_vector(2 downto 0);
  signal runofn 	: std_logic_vector(2 downto 0) := (others => '0');
  signal res1 		: unsigned(31 downto 0);
  signal res2 		: unsigned(31 downto 0);
  signal x_new		: unsigned(16 downto 0);
  signal x   		: unsigned(15 downto 0) := (others => '0');  --integrator
  signal s   		: unsigned(15 downto 0) := to_unsigned(40, 16);  --syllabic (s_min)
